  ...
  return (
    <Container style={{ paddingTop: "90px" }}>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Search notes" variant="outlined" fullWidth />}
      />
      ...
      <Stack direction="column" spacing={1} style={{ marginTop: "10px" }}>
        {notes.map((note) => (
          <NoteCard key={note.id} id={note.id} content={note.content} title={note.title} tags={note.tags} />
        ))}
      </Stack>
    </Container>
  );
