...
const FlashCardSaveWizard: React.FC<FlashCardSaveWizardProps> = ({
  discussionId,
  messageId,
  open,
  setOpen,
  ...
  const loadFlashcards = () => {
    ...
    fetch(
      BACKEND_URL +
        `/api/discussions/${discussionId}/messages/${messageId}/flashcards`,
      {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
        ...
        <DialogContent>
          ...
          {activeStep === 0 && (
            <div>
              <h2>{statusText}</h2>
              {isThinking && <LinearProgress sx={{ marginBottom: 2 }} />}
              {/* <div >
