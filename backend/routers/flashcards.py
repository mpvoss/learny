...
def filter_flashcards_due(flashcards: List[FlashCard], current_date: datetime.date):
    ...
    for flashcard in flashcards:
        if not flashcard.last_reviewed_date:
            ...
            flashcard.last_reviewed_date + datetime.timedelta(days=flashcard.interval)
            <= current_date
        ):
            to_review.append(flashcard)
    return to_review
...
def save_flashcards(
    flashcardSaveRequest: FlashcardSaveRequest,
    db: Session = Depends(get_db),
    tag: List[str] = Query(None),
    current_user: User = Depends(get_current_user),
):
    ...
    db_tag = (
        db.query(Tag)
        .filter(Tag.name == new_tag.name, Tag.user_id == current_user.id)
        .first()
    )
...
def review_flashcard(
    id: int,
    flascardReview: FlashcardReview,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    flashcard = (
    ...
    if not flashcard.last_reviewed_date:
        ...
        stats = review(
            quality, flashcard.easiness_factor, flashcard.interval, flashcard.repetition
        )
